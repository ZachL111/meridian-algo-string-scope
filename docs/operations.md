...
`meridian-algo-string-scope` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for string analysis with negative fixtures, human-readable error snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 156. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 4.
